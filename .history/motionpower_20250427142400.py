...
# ✅ Use Hardware SPI pins (SCLK for Clock, MOSI for Data)
...
last_color = (0, 0, 0)  # Start with LEDs off
manual_override = False  # Remote page color override
...
def blink_leds_once():
    set_all_leds((255, 255, 255))  # Full white blink
    time.sleep(0.2)
    set_all_leds((0, 0, 0))        # Turn off after flash
    time.sleep(0.8)
...
# --- Initialize LEDs all off at start ---
set_all_leds((0, 0, 0))

try:
    while True:
        # --- Check for color change from Flask ---
        try:
            ...
            pass  # color.txt not created yet; ignore

        # --- Distance sensor / motion logic ---
        ...
        if distance is not None and distance <= TRIGGER_DISTANCE:
            ...
            if blink_count >= 5:
                blinking = False
                # After blinking, restore manual color if set
                if manual_override:
                    try:
                        with open(COLOR_FILE, 'r') as f:
                            color_hex = f.read().strip()
            ...
            if not display_on and now - start_time >= STAND_TIME_REQUIRED:
                print("Person stayed 3 seconds: turning display ON.")
